...
class QFunction:
    ...
    def __init__(self, model: mm.Model):
        ...
        self.build_state_action_space()
        # We initialise the value function to 0 for each state in the state space.
        self.values = {}
        self.initialise_values()

    def build_state_action_space(self):
        tuples_list = []
        # We loop through all possible states
        for arrival_numbers in itertools.product(range(int(self.model.capacity) + 1),
                                                 repeat=len(self.complete_arrival_graph_edges_list)):
            state = mm.State.zeros(matching_graph=self.model.matching_graph, capacity=self.model.capacity)
            try:
                ...
                state_tuple = tuple(state.data)
                if state_tuple not in tuples_list:
                    # We loop through all possible arrivals
                    for arrival_pair in self.complete_arrival_graph_edges_list:
                        arrivals = mm.State.zeros(matching_graph=self.model.matching_graph,
                                                  capacity=self.model.capacity)
                        arrivals[arrival_pair] += 1.
                        # We loop through all possible matchings
                        total_state = state + arrivals
                        max_matchings_ranges = [range(total_state[matching_edge].max())
                                                for matching_edge in self.model.matching_graph.edges]
                        for matching_numbers in itertools.product(*max_matchings_ranges):
                            try:
                                matchings = mm.Matching(state=total_state, values=matching_numbers)
                            except ValueError:
                                pass
                            else:
                                self.state_action_space.append((state, arrivals, matchings))
                    tuples_list.append(tuple(state.data))

    def initialise_values(self):
        for state, arrivals, matchings in self.state_action_space:
            self[state, arrivals, matchings] = 0.

    def __getitem__(self, item: Tuple[mm.State, mm.State, mm.Matching]):
        state, arrivals, matchings = item
        if type(state) == mm.State and type(arrivals) == mm.State and type(matchings) == mm.Matching:
            return self.values[(tuple(state.data), tuple(arrivals.data), tuple(matchings.to_nodesdata()))]
        else:
            return NotImplemented

    def __setitem__(self, item: Tuple[mm.State, mm.State, mm.Matching], value):
        state, arrivals, matchings = item
        if type(state) == mm.State and type(arrivals) == mm.State and type(matchings) == mm.Matching:
            self.values[(tuple(state.data), tuple(arrivals.data), tuple(matchings.to_nodesdata()))] = value
        else:
            return NotImplemented

    def copy(self):
        new_q_function = QFunction(model=self.model)
        for state, arrivals, matchings in self.state_action_space:
            new_q_function[state, arrivals, matchings] = self[state, arrivals, matchings]
        return new_q_function
    ...
    def run(self, nb_iterations=None, save_file=None, plot=False):
        ...
        if plot:
            if nb_iterations is None:
                ...
                iterator = np.arange(nb_iterations)
                iterator = utils.progprint(iterator=iter(iterator), total=nb_iterations, perline=10,
                                           periteration=1, show_times=True)
...
class LinearTimeScale(TimeScale):

    def __init__(self, start=1., step=0.01, min_value=0.01):
        self.start = start
        self.step = step
        self.min_value = min_value

    def __getitem__(self, n):
        return np.maximum(self.start - self.step * float(n), self.min_value)
...
class SalmutD:
    ...
    def __init__(self, model: mm.Model, fast_time_scale: TimeScale, slow_time_scale: TimeScale,
                 approximation_time_scale: TimeScale, sigma: float):
        self.model = model
        ...
        self.approximation_time_scale = approximation_time_scale
        self.sigma = sigma
    ...
    def iterate_critic(self, current_state: mm.State, current_arrivals: mm.State):
        ...
        critic_error = current_costs + self.V[next_state, next_arrivals] - self.V[self.state_ast, self.arrivals_ast] \
            - self.V[current_state, current_arrivals]
        # We update the critic through Relative Value Iteration Algorithm
        self.V[current_state, current_arrivals] += self.fast_time_scale[current_occurence_count] * critic_error
        return next_state, next_arrivals, critic_error
    ...
    def iterate(self, current_state: mm.State, current_arrivals: mm.State, iteration):
        ...
        # Simulated annealing for sigma, parameter of the approximation for the policy
        self.threshold_policy.sigma = self.sigma * self.approximation_time_scale[iteration]
        # We update the critic
        next_state, next_arrivals, critic_error = self.iterate_critic(current_state=current_state,
    ...
    def run(self, nb_iterations=None, plot=False, verbose=False):
        ...
        if plot:
            if nb_iterations is None:
                ...
                iteration = 1.
                while not self.is_optimal():
                    current_state, current_arrivals, critic_error = self.iterate(
                        current_state=current_state, current_arrivals=current_arrivals, iteration=iteration)
                    threshold_traj.append(self.threshold)
                    iteration += 1.
                ...
                for iteration in iterator:
                    ...
                    if verbose:
                        critic_evaluation_traj[iteration] = critic_error
            ...
            if nb_iterations is None:
                ...
                for iteration in np.arange(1, nb_iterations + 1):
                    current_state, current_arrivals, critic_error = self.iterate(
                        current_state=current_state, current_arrivals=current_arrivals, iteration=iteration)
                    # print("iteration: {}, threshold: {}".format(iteration, self.threshold_policy.threshold))
        return self.threshold, threshold_traj
...
class SalmutDB(SalmutD):
    ...
    def compute_grad_action_smoother(self, matching_threshold):
        sigma = self.threshold_policy.sigma
...
class SalmutDBWithoutOccurrences(SalmutDB):
    # Compared to SalmutDB, we remove occurences
    ...
    def iterate_critic(self, current_state: mm.State, current_arrivals: mm.State, iteration):
        ...
        critic_error = current_costs + self.V[next_state, next_arrivals] - self.V[self.state_ast, self.arrivals_ast] \
                       - self.V[current_state, current_arrivals]
        # We update the critic through Relative Value Iteration Algorithm
        self.V[current_state, current_arrivals] += self.fast_time_scale[iteration] * critic_error
        return next_state, next_arrivals, critic_error
...
class SalmutDBcriticRVI(SalmutDB):
    # Compared to SalmutDB, we use a RVI for the critic

    def iterate_critic(self, current_state: mm.State, current_arrivals: mm.State):
        current_occurence_count = self.occurrence_count[current_state, current_arrivals]
        operator = 0.
        if np.any(current_state.data + current_arrivals.data > self.model.capacity):
            total_state = current_state.copy()
            operator += self.model.penalty
        else:
            total_state = current_state + current_arrivals
        operator += np.dot(self.model.costs.data, total_state.data)
        for matching, matching_probability in self.threshold_policy.compute_distribution_state(state=total_state):
            for arrival_edge in self.V.complete_arrival_graph_edges_list:
                arrival_probability = np.prod(self.model.arrival_dist[arrival_edge])
                arrival = mm.State.zeros(self.model.matching_graph, self.model.capacity)
                arrival[arrival_edge] += 1.
                operator += self.V[total_state - matching, arrival] * arrival_probability * matching_probability
        operator -= self.V[self.state_ast, self.arrivals_ast]
        critic_error = operator - self.V[current_state, current_arrivals]
        # We update the critic through Relative Value Iteration Algorithm
        self.V[current_state, current_arrivals] += self.fast_time_scale[current_occurence_count] * critic_error
        return critic_error
    ...
    def iterate(self, current_state: mm.State, current_arrivals: mm.State, iteration):
        ...
        # Simulated annealing for sigma, parameter of the approximation for the policy
        # if iteration >= 50000:
        #     self.threshold_policy.sigma = 0.25
        self.threshold_policy.sigma = self.sigma * self.approximation_time_scale[iteration]
        # We sample the next state and arrivals and get the costs of the current state and arrivals
        next_state_list, next_arrivals, costs_list = self.model.iterate_state_with_arrival(
            states_list=[current_state.copy()], arrivals=current_arrivals.copy(), policies=[self.threshold_policy])
        next_state = next_state_list[0]
        # We update the critic
        critic_error = self.iterate_critic(current_state=current_state, current_arrivals=current_arrivals)
        # We update the actor
        self.iterate_actor(current_state=current_state, current_arrivals=current_arrivals, iteration=iteration)
        # We update the occurrence count
        self.occurrence_count[next_state, next_arrivals] += 1.
...
class SalmutDBcriticRVIactorQ(SalmutDBcriticRVI):
    # Compared to SalmutDBcriticRVI, we update the actor using the formula based on the Q function from Sutton
    ...
    def iterate_actor(self, current_state: mm.State, current_arrivals: mm.State, iteration: float):
        ...
        if np.any(current_state.data + current_arrivals.data > self.model.capacity):
            current_arrivals = mm.State.zeros(matching_graph=self.model.matching_graph, capacity=self.model.capacity)
        current_total_state = current_state + current_arrivals
        current_cost = np.dot(current_total_state.data, self.model.costs.data.T)
        # We sample the action chosen (match the threshold edge or not) and compute the new state
        actor_matching = mm.Matching.zeros(current_total_state)
        actor_matching[1, 1] += current_total_state[1, 1].min()
        actor_matching[2, 2] += current_total_state[2, 2].min()
        ...
        # We update the actor through gradient based method
        new_threshold = self.threshold - self.slow_time_scale[iteration] \
            * np.sum([grad_action_smoother * (current_cost - self.V[self.state_ast, self.arrivals_ast]
                                              + self.V[actor_state, actor_arrival])
                      for (actor_state, grad_action_smoother) in zip(actor_state_list, grad_action_smoother_list)])
        self.threshold = np.minimum(np.maximum(0.01, new_threshold), self.model.capacity - 0.01)
        self.threshold_policy.threshold = self.threshold


class SalmutDBactorQ(SalmutDB):
    # Compared to SalmutDBcriticRVI, we update the actor using the formula based on the Q function from Sutton
...
class SalmutDBCheatExploActor(SalmutDB):
    # We replace the explo policy by a deterministic exploration

    def __init__(self, model: mm.Model, fast_time_scale: TimeScale, slow_time_scale: TimeScale,
                 approximation_time_scale: TimeScale, sigma: float):
        super(SalmutDBCheatExploActor, self).__init__(model=model, fast_time_scale=fast_time_scale,
                                                      slow_time_scale=slow_time_scale,
                                                      approximation_time_scale=approximation_time_scale, sigma=sigma)
        self.explo_index = 0

    def iterate_critic(self, current_state: mm.State, current_arrivals: mm.State, current_costs, next_state: mm.State,
                       next_arrivals: mm.State):
        current_occurence_count = self.occurrence_count[current_state, current_arrivals]
        # We update the critic through Relative Value Iteration Algorithm
        self.V[current_state, current_arrivals] = (1. - self.fast_time_scale[current_occurence_count]) \
            * self.V[current_state, current_arrivals] + self.fast_time_scale[current_occurence_count] \
    ...
    def iterate(self, current_actor_state: mm.State, current_actor_arrivals: mm.State, iteration):
        # Simulated annealing for sigma, parameter of the approximation for the policy
        self.threshold_policy.sigma = self.sigma * self.approximation_time_scale[iteration]

        # We sample the current explo state and arrivals
        current_explo_state, current_explo_arrivals = self.V.state_space[self.explo_index]
        self.explo_index = (self.explo_index + 1) % len(self.V.state_space)
        # We sample the next explo state and get the reward of the current explo state
        next_critic_state_list, next_critic_arrivals, explo_costs_list = self.model.iterate_state_with_arrival(
            states_list=[current_explo_state.copy()], arrivals=current_explo_arrivals.copy(),
            policies=[self.threshold_policy])
        next_critic_state = next_critic_state_list[0]
        current_explo_costs = explo_costs_list[0]
        # We update the critic
        self.iterate_critic(current_state=current_explo_state, current_arrivals=current_explo_arrivals,
                            current_costs=current_explo_costs, next_state=next_critic_state,
                            next_arrivals=next_critic_arrivals)

        # We sample the next actor state and actor arrivals and get the costs of the current state and arrivals
        next_actor_state_list, next_actor_arrivals, actor_costs_list = self.model.iterate_state_with_arrival(
            states_list=[current_actor_state.copy()], arrivals=current_actor_arrivals.copy(),
            policies=[self.threshold_policy])
        next_actor_state = next_actor_state_list[0]
        current_actor_costs = actor_costs_list[0]
        # We update the actor
        self.iterate_actor(current_state=current_actor_state, current_arrivals=current_actor_arrivals,
                           iteration=iteration)
        # We update the occurrence count
        self.occurrence_count[current_explo_state, current_explo_arrivals] += 1.
        return next_actor_state, next_actor_arrivals
    ...
    def run(self, nb_iterations=None, plot=False, verbose=False):
        ...
        if plot:
            if nb_iterations is None:
                ...
                iteration = 1.
                while not self.is_optimal():
                    current_state, current_arrivals = self.iterate(current_actor_state=current_state,
                                                                   current_actor_arrivals=current_arrivals,
                                                                   iteration=iteration)
                    threshold_traj.append(self.threshold)
                    iteration += 1.
                threshold_traj = np.array(threshold_traj)
...
class ActorCriticDiscounted:
    ...
    def iterate_critic(self, current_state: mm.State, current_arrivals: mm.State, current_costs: float,
                       next_state: mm.State, next_arrivals: mm.State):
        current_occurence_count = self.occurrence_count[current_state, current_arrivals]
        critic_error = current_costs + self.model.discount * self.V[next_state, next_arrivals] \
            - self.V[current_state, current_arrivals]
        # We update the critic through Relative Value Iteration Algorithm
        self.V[current_state, current_arrivals] += self.fast_time_scale[current_occurence_count] \
            * critic_error
    ...
    def iterate_actor(self, matching_threshold: float, matching_probability: float, iteration: float,
                      critic_error: float):
        grad_action_smoother = self.compute_grad_action_smoother(matching_threshold=matching_threshold)
        new_threshold = self.threshold - self.slow_time_scale[iteration] \
            * critic_error * grad_action_smoother / matching_probability
        self.threshold = np.minimum(np.maximum(0.01, new_threshold), self.model.capacity - 0.01)
        self.threshold_policy.threshold = self.threshold
    ...
    def iterate(self, current_state: mm.State, current_arrivals: mm.State, iteration):
        ...
        # We sample the next state and arrivals and get the costs of the current state and arrivals
        if np.any(current_state.data + current_arrivals.data > self.model.capacity):
            current_total_state = current_state.copy()
        else:
            current_total_state = current_state + current_arrivals
        current_matchings, matching_threshold, matching_probability = self.threshold_policy.match_and_threshold(
            state=current_total_state)
        next_state, next_arrivals, costs = self.model.iterate_state_with_arrival_and_matching(
            state=current_state.copy(), arrivals=current_arrivals.copy(), matchings=current_matchings)
        # Simulated annealing for sigma, parameter of the approximation for the policy
        self.threshold_policy.sigma = self.sigma * self.approximation_time_scale[iteration]
        # We update the critic
        critic_error = self.iterate_critic(current_state=current_state, current_arrivals=current_arrivals,
                                           current_costs=costs, next_state=next_state, next_arrivals=next_arrivals)
        # We update the actor
        self.iterate_actor(matching_threshold=matching_threshold, matching_probability=matching_probability,
                           iteration=iteration, critic_error=critic_error)
        # We update the occurrence count
        self.occurrence_count[next_state, next_arrivals] += 1.
        return next_state, next_arrivals, critic_error
    ...
    def evaluate_critic(self):
        ...
        for i, (state, arrivals) in enumerate(self.V.state_space):
            ...
            for arrival_edge in self.V.complete_arrival_graph_edges_list:
                ...
                arrival = mm.State.zeros(self.model.matching_graph, self.model.capacity)
                arrival[arrival_edge] += 1.
                bellman_operator += self.model.discount * self.V[new_state - matching, arrival] * arrival_probability
            # We compute the difference in the fixed point Bellman equation
            fixed_point_diff[i] = self.V[state, arrivals] - bellman_operator
        return np.linalg.norm(fixed_point_diff, ord=np.inf)
...
class SalmutDBactorQDiscounted:
    ...
    def iterate_actor(self, current_state: mm.State, current_arrivals: mm.State, iteration: float):
        ...
        # We update the actor through gradient based method
        new_threshold = self.threshold - self.slow_time_scale[iteration] \
                        * np.sum([grad_action_smoother * (current_cost +
                                                          self.model.discount * self.V[actor_state, actor_arrival])
                                  for (actor_state, grad_action_smoother) in
                                  zip(actor_state_list, grad_action_smoother_list)])
        self.threshold = np.minimum(np.maximum(0.01, new_threshold), self.model.capacity - 0.01)
        self.threshold_policy.threshold = self.threshold
            ...
            if nb_iterations is None:
                ...
                for iteration in iterator:
                    try:
                        ...
                        print(f"An exception occured at iteration {iteration} with the threshold value of {self.threshold} and with the approximation parameter value of {self.threshold_policy.sigma}.")
                        break
                    threshold_traj[iteration] = self.threshold
                    if verbose:
                        critic_evaluation_traj[iteration] = critic_error
