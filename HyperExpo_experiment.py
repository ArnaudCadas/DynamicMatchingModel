...
class ExpoPolicy(RandomizedAdmissionPolicy):
    ...
    def not_admitting_probability(self, state, phase, arrival_type):
        return np.exp(state - self.thresholds[arrival_type - 1, phase - 1]) \
            / (1. + np.exp(state - self.thresholds[arrival_type - 1, phase - 1]))

    def not_admitting_grad(self, state, phase, arrival_type):
        return - np.exp(state - self.thresholds[arrival_type - 1, phase - 1]) \
               / np.power((1. + np.exp(state - self.thresholds[arrival_type - 1, phase - 1])),
                          2.)
...
class AdmissionControlModel:
    ...
    def event_distribution(self, state: int, phase: int):
        ...
        if state > 0:
            ...
            distribution = distribution_unormalized / np.sum(distribution_unormalized)
        else:
            distribution_unormalized = np.array(self.arrival_rates)
            distribution = distribution_unormalized / np.sum(distribution_unormalized)
        return distribution
...
def demo_salmut():
    np.random.seed(42)
    model = ModelA().model
    thresholds_init = np.array([[0., 0.], [0., 0.]])
    ...
    algo = SalmutCWithRVICriticExploActor(model=model, fast_time_scale=fast_time_scale, slow_time_scale=slow_time_scale,
