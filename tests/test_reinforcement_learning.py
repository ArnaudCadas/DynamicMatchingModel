...
class TestValueFunction:
    ...
    N_model = Model.Model(matching_graph=N_graph, arrival_dist=arrival_dist, costs=costs, x_0=x0, capacity=capacity,
                          penalty=100.)
    ...
    def test_build_state_space(self):
        N_value_function_a = RL.ValueFunction(model=TestValueFunction.N_model)
        state_space_theory_tuples = [(0., 0., 0., 0.), (0., 1., 0., 1.), (0., 2., 0., 2.), (0., 1., 1., 0.),
                                     (0., 2., 1., 1.), (0., 2., 2., 0.), (1., 0., 0., 1.), (1., 1., 0., 2.),
                                     (1., 1., 1., 1.), (1., 2., 1., 2.), (1., 2., 2., 1.), (2., 0., 0., 2.),
                                     (2., 1., 1., 2.), (2., 2., 2., 2.), (1., 0., 1., 0.), (1., 1., 2., 0.),
                                     (2., 0., 1., 1.), (2., 1., 2., 1.), (2., 0., 2., 0.)]
        state_space_theory = [Model.State(
            values=np.array(state_tuple), matching_graph=TestValueFunction.N_graph,
            capacity=TestValueFunction.capacity) for state_tuple in state_space_theory_tuples]
...
class TestValueIteration:
    ...
    N_model = Model.Model(matching_graph=N_graph, arrival_dist=arrival_dist, costs=costs, x_0=x0, capacity=10.,
                          penalty=100.)
